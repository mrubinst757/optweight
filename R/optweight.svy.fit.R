optweight.svy.fit <- function(covs, tols = 0, targets, target_n = NULL, s.weights = NULL, norm = "l2", std.binary = FALSE, std.cont = TRUE, min.w = 1E-8, verbose = FALSE, sigma2.y = 1, p = 1, re = 0, group_n = NULL, exp1 = TRUE, exp2 = FALSE, ...) {
  args <- list(...)
  ...
  if (norm == "l2") {
    ...
    if(exp1 == TRUE) {
     ...
     P2 = sparseMatrix(1:N, 1:N, x = 2*sigma2.y)
     P  = P1 + P2
     q  = rep(0, N)
    }  
    if(exp2 == TRUE) {
